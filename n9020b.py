...
class N9020B(object):
	...
	def __init__(self, ip=DEVICE_IP, port=DEVICE_PORT):
		...
		self._connection_command = 'telnet {0} {1}'.format(self.ip, self.port)
	...
	def command(self, cmd):
		...
		self.get_prompt()
		data = self.connection.before
		data = data.split(this_command)[-1].strip()
		return data
	
	def rlist_to_vectors(self, rlist):
		fparray = np.array(map(float,rlist.strip().split(',')))
	...
	def get_correction(self, cnum=1):
		data = self.command(':CORR:CSET{0}:DATA?'.format(cnum))
		return self.rlist_to_vectors(data)
	...
	def mark_to_max(self, marknum=1):
		self.command(':CALC:MARK{0}:MAX'.format(marknum))
        def mark_to_max_next(self, marknum):
                self.command(':CALC:MARK{0}:MAX:NEXT'.format(marknum))

		
	def get_marker_y(self, marknum=1):
		return float(self.command(':CALC:MARK{0}:Y?'.format(marknum)))
	
	def get_marker_x(self, marknum=1):
		return self.hz_to_freq_units(float(self.command(':CALC:MARK{0}:X?'.format(marknum))))
		
	def get_marker_pos(self, marknum=1):
		x = self.hz_to_freq_units(float(self.command(':CALC:MARK{0}:X?'.format(marknum))))
		y = float(self.command(':CALC:MARK{0}:Y?'.format(marknum)))
		return x,y 
		
	def set_center_frequency(self, freq):
		self.command(':SENS:FREQ:CENT {0} {1}'.format(freq, self.freq_units))
	
	def set_start_frequency(self, freq):
		self.command(':SENS:FREQ:STAR {0} {1}'.format(freq, self.freq_units))
		
	def set_stop_frequency(self, freq):
		self.command(':SENS:FREQ:STOP {0} {1}'.format(freq, self.freq_units))
		
	def get_center_frequency(self):
		return self.hz_to_freq_units(self.command(':SENS:FREQ:CENT?'))
	
	def set_span(self, span):
		self.command(':SENS:FREQ:SPAN {0} {1}'.format(span, self.freq_units))
	...
	def set_rbw(self, bw):
		self.command('SENS:BAND:RES {0} {1}'.format(bw, self.freq_units))
	...
	def set_sweep_time(self, time):
		...
		self.command('SENS:SWE:TIME {0}'.format(time))
	...
	def set_sweep_points(self, points):
		self.command('SENS:SWE:POIN {0}'.format(points))
	...
	def set_trace_type(self, ttype='writ', trace_num=1):
		...
		self.command('TRAC{0}:TYPE {1}'.format(trace_num, ttype.upper()))
		
	def get_trace_type(self, trace_num=1):
		'''See page 833'''
		return self.command('TRAC{0}:TYPE?'.format(trace_num))
	...
	def set_detector_type(self, ttype='norm', trace_num=1):
		...
		self.command('DET:TRAC{0} {1}'.format(trace_num, ttype.upper()))
	
	def get_detector_type(self, trace_num=1):
		'''See page 840'''
		return self.command('DET:TRAC{0}?'.format(trace_num))
		
	def set_average_hold_count(self, num):
		self.command(':AVER:COUN {0}'.format(int(num)))
...
def fast_peak_meas(device=None, center_freq = 145000, span = 5000, time_to_measure = 0, saveFile = None):
	...
	try:
		while running:
			try:
				f, a = map(float,device.get_marker_table().split(',')[2:4])
			except ValueError:
