...
paramsReader = ConfigParser.ConfigParser()
paramsReader.read(os.path.dirname(os.path.realpath(__file__))+"/robotPosData.ini")
if paramsReader.has_section("params"):
     y_source=paramsReader.getint("params", "y_source")
     x_source=paramsReader.getint("params", "x_source")
     L1=paramsReader.getint("params", "L1")
     L2=paramsReader.getint("params", "L2")
#array inicial
...
class Menu:
    ...
    def update_status(self, robot):
        with self.terminal.hidden_cursor():
            ...
            print self.terminal.move(self.status_pos[0]+6,self.status_pos[1]+3) + "Eje X actuador respesto a la fuente: " + str(robot.MotorX.coord-x_source) + " mm"
            print self.terminal.move(self.status_pos[0]+7,self.status_pos[1]+3) + "Eje Y actuador respecto a la fuente: " + str(robot.MotorY.coord-y_source) + " mm"
            x_s=robot.MotorX.coord-x_source-L2*np.cos(np.radians(-robot.MotorPhi.coord))-L1*np.sin(np.radians(-robot.MotorPhi.coord))
            y_s=robot.MotorY.coord-y_source+L1*np.cos(np.radians(-robot.MotorPhi.coord))-L2*np.sin(np.radians(-robot.MotorPhi.coord))   
...
def ref_radio(some_menu):
    ...
    # Posición inicial del sistema
     ...
     with some_menu.terminal.raw():
        ...
        robotmmwave.moveTo('x',r+x_source+L2)
        robotmmwave.moveTo('y',y_source-L1)
        some_menu.change_menu("esf")
        some_menu.actual_status=some_menu.predefined_messages["wait"]
        some_menu.update_status(robotmmwave)
...
def move_sphere(some_menu):
    ...
    # Posición inicial del sistema
     ...
     with some_menu.terminal.raw():
        ...
        robotmmwave.moveTo('x',radio+x_source+L2+xmm)
        robotmmwave.moveTo('y', y_source-L1 + ymm)
        some_menu.change_menu("esf")
        some_menu.actual_status=some_menu.predefined_messages["wait"]
        some_menu.update_status(robotmmwave)
